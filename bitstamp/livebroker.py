...
def build_order_from_open_order(openOrder, instrumentTraits, instrument):
    if openOrder.isBuy():
...
class LiveBroker(broker.Broker):
    ...
    def __init__(self, clientId, key, secret, instrument="BTC", currency="USD"):
    ...
    def refreshAccountBalance(self):
        """Refreshes cash and Coin balance."""
        ...
        # Cash
        self.__cash = round(balance.getCurrencyAvailable(self.__currency), 2)
        common.logger.info("%s %s" % (self.__cash, self.__currency))

        # Coins
        coin = balance.getCoinAvailable(self.__instrument)
        if coin:
            self.__shares = {self.__instrument: coin}
        else:
            self.__shares = {}
        common.logger.info("%s %s" % (coin, self.__currency))
    ...
    def refreshOpenOrders(self):
        ...
        for openOrder in openOrders:
            self._registerOrder(build_order_from_open_order(openOrder, self.getInstrumentTraits(self.__instrument)))
    ...
    def _onUserTrades(self, trades):
        for trade in trades:
            ...
            if order is not None:
                fee = trade.getFee()
                fillPrice = trade.getCoinCurrency(self.__instrument, self.__currency)
                coinAmount = trade.getCoin(self.__instrument)
                dateTime = trade.getDateTime()

                # Update cash and shares.
                self.refreshAccountBalance()
                # Update the order.
                orderExecutionInfo = broker.OrderExecutionInfo(fillPrice, abs(coinAmount), fee, dateTime)
