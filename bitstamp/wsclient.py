...
class WebSocketClient(pusher.WebSocketClient):
    ...
    def __init__(self, queue, instrument, currency):
    ...
    def onMessage(self, msg):
        ...
        if self.__instrument == "btc" and self.__currency == "usd":
            msg_channel = "order_book"
        else:
            msg_channel = "order_book_" + self.__instrument + self.__currency

        if event == "trade":
            self.onTrade(Trade(get_current_datetime(), msg))
        elif event == "data" and msg.get("channel") == msg_channel:
    ...
    def onConnectionEstablished(self, event):
        ...
        if self.__instrument == "btc" and self.__currency == "usd":
            ...
            channels = ["live_trades_" + self.__instrument + self.__currency, "order_book_" + self.__instrument + self.__currency]
...
class WebSocketClientThread(client.WebSocketClientThreadBase):
    ...
    def run(self):
        ...
        try:
            self.__wsClient = WebSocketClient(self.__queue, self.__instrument, self.__currency)
